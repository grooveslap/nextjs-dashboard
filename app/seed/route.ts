...
async function seedUsers() {
    ...
    const collection = client.db('dashboard').collection('users');
...
async function seedCustomers() {
  const client = await clientPromise;
  const collection = client.db('dashboard').collection('customers');
...
async function seedInvoices() {
  const client = await clientPromise;
  const collection = client.db('dashboard').collection('invoices');
